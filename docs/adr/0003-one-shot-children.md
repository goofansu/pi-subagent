...
Accepted. This ADR's no-mid-run-guidance consequence is superseded: an active
Run accepts bounded guidance. [ADR-0013](0013-stable-subagent-identity.md) adds
stable Session-scoped identity and idle adapter retention above Runs, and
[ADR-0014](0014-controlled-agent-resume.md) supersedes only this ADR's
rejection of follow-up orchestration: resume creates a new one-shot Run rather
than reopening the old one. [ADR-0017](0017-retained-pi-sdk-conversation.md)
and [ADR-0018](0018-ordered-claude-query-conversation.md) supersede this ADR's
Pi and Claude process and no-guidance consequences while preserving that same
Run boundary.
...
spawn (`agent_start`), join (`agent_wait`), cancel (`agent_cancel`). Bounded
guidance to an active Run was added later without retaining an idle child or
creating a resumable provider session.

## Consequences

At this decision, a Run required cancellation and a new Run to correct its
direction. ADR-0013 retains the Subagent-scoped adapter after settlement, not
the child process or the Run's Control mailbox; ADR-0017 and ADR-0018 supersede
the historical Pi and Claude limitation.
