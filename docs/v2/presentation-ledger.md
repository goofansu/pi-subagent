...
| Difference | Classification |
| --- | --- |
| Twelve row fixtures — the description tail, a reported activity tail, no turns yet, a truncated long agent name, each of the three backends, each of the three terminal phases, three Runs with the summary line, ten Runs with the overflow line, and a narrow terminal that drops turn accounting | **All identical.** Byte for byte — *at the one instant each pair was drawn at*, which is less than it sounds and is discussed in the entry below. |
...
| A settled row's *duration* | **A parity break this comparison could not see, found later by watching four Subagents finish and fixed since.** v1's row view carried the elapsed figure itself, computed against the run's recorded finish time once it had settled. v2's published row carried only the instant a Run *started*, so a settled row's duration was `now` minus that — recomputed on every redraw, and therefore climbing for as long as the row waited for its notice to land. A row read `completed in 11.6s`, then `12.4s`, then `1m 3s`, without the Run doing anything in between: the figure was the Run's age at the moment of the draw rather than its cost. The published row now carries the instant the Run settled, and presentation measures a terminal row against it. Recorded in [the soak record](soak.md#the-settled-rows-climbing-duration-2026-09-03-severity-3). |
| *What the twelve identical row fixtures did and did not establish* | **A limitation of the harness, recorded so the entry above is not read as a contradiction.** Each pair was rendered once, at one supplied instant. A duration that has stopped and a duration that is still climbing are the same string when you only look once, so every one of those twelve pairs could be — and one was — identical and wrong together. What the comparison established is that the two trees *lay a row out* the same way, not that a row says the same thing a minute later. The preamble's own note that v1's view carries an elapsed figure where v2 derives one from a start and a supplied `now` was the visible edge of this, and was not followed up at the time. Proving it needs two looks, which is now what the widget's host test and the row's golden both take. |
