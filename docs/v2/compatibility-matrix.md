...
| **Empty description** | **[2.0 close]** Rejected before admission is entered: the description is the Run's Label, and a description that is empty or that bounds away to nothing has no label in it. Nothing is reserved, nothing is claimed, and no identifier is spent. v1 had no lower bound and would have labelled the Run `""` — [operation semantics §2](operation-semantics.md#2-start-and-resume-admission-are-atomic). | Same. | Same. |
...
| **empty description** | `agent_start refuses an empty description, and spends no identifier doing it` (`host/tools.test.ts`) | Backend-independent: the same test | Backend-independent: the same test |
...
| **Empty description** | **[2.0 close]** Rejected the same way `agent_start` is, and for the same reason: a resumed Run gets its own Label from its own description. Nothing is queued and the Subagent's one-active-Run claim is never taken, so the Subagent stays resumable. | Same. | Same. |
...
| **During shutdown** | Rejected; shutdown wins admission synchronously and late settlement cannot reopen the Subagent. **[2.0 close]** With one exception in which sentence a caller reads: an empty description is refused before admission is entered, so a shutting-down Session answers a call with no description with `empty label` rather than `shutting down`. Both are refusals that start nothing and queue nothing. **[v2 change]** v1 reports this as `unknown subagent`; v2 reports a distinct shutting-down outcome — [operation semantics §5](operation-semantics.md#5-shutdown-rejects-new-work-as-soon-as-it-begins). | Same. | Same. |
...
| **empty description** | `agent_resume refuses an empty description, and the Subagent stays resumable` (`host/tools.test.ts`) | Backend-independent: the same test | Backend-independent: the same test |
...
| **The pointer** | **[Phase A; wording revised in Phase C]** Present for every terminal status, as `The result is available.` / `Partial output is available.` / `No output was produced. The Run record is available.`, each followed by `Call agent_result with {"id":"<runId>"}.` in its own sentence — the exact argument shape, so the parent copies rather than composes. The three say what a model will *find*: availability is read off the Result's output, and describes the stored Result rather than the Run's success, so a completed Run whose output bounding cut is still `complete` and one with no output at all is `record-only`. [Semantics §5](../v2-simplify/notification-semantics.md#pointer), derivation in [§3](../v2-simplify/notification-semantics.md#3-result-availability). | Same. | Same. |
| **The Run label** | **[Phase A; lower bound added at the 2.0 close]** **Two** bounds, applied once at admission, each taking one of contributing invariant 11's two branches. The **upper** bound — one line, at most 200 bytes — truncates and records: a shortened label is a Run diagnostic rather than a refusal, because a long label can be cut honestly. The **lower** bound — non-empty — refuses the start or the resume with the typed outcome `empty label`, because there is nothing to shorten and a substituted label would be the surface inventing a fact about the Run. So no Run can carry an empty label, and no notice header, collapsed line or widget row can show one. Both tool schemas state both bounds. [Semantics §4](../v2-simplify/notification-semantics.md#4-the-run-label). | Same. | Same. |
...
| **Collapsed transcript line** | **[Phase A]** `<agent> · <label> · <verb> in <duration>`. No id, no character count, and **no cost** — cost is not backend-independent, so a cost here would read as a fact about spending and be a fact about the backend. The ids are in the expanded text; the spend is on the notice's accounting line. [Semantics §6](../v2-simplify/notification-semantics.md#6-what-the-human-sees). | Same, and identical: this is one of the cells where "Same." is now literally true rather than true of the shape. | Same. |
...
| **the Run label's bound** | `a label past its byte bound is collapsed to one line, cut, and recorded`, `a label within its bound is stored whole and records nothing`, `a maximal label leaves a result inside its byte budget once everything removable is cut` (`runtime/bounds.test.ts`); `T1: the label's bound is stated on both description fields`, `T1: both description fields say the field is never empty` (`host/tool-schemas.test.ts`); the empty end: `a start and a resume refuse an empty description in their own family's words` (`presentation/prose.test.ts`), `agent_start refuses an empty description, and spends no identifier doing it`, `agent_resume refuses an empty description, and the Subagent stays resumable` (`host/tools.test.ts`) | Same. | Same. |
...
| **the collapsed line** | `S-1: a collapsed notice names the agent, the task, and the outcome`, `S-1: the collapsed line reads the same whichever backend ran the Run`, `S-2: a failed and a cancelled summary read the same way, with the verb changed`, `a collapsed notice carries no id and no character count`, `the whole collapsed line is fitted to its width, and the label is what gives`, `a line too narrow for any label drops the label whole, not into a gap`, `a label is capped even when the line has room to spare` (`presentation/renderers.test.ts`); `the collapsed line is fitted to the width the terminal actually gave` (`host/notification-message.test.ts`); `S-1: the ids are in the expanded text, where a tool call needs them`, `the details carry the collapsed line and the ids, never the text`, `a message missing a field the collapsed line needs is rejected` (`host/notification-message.test.ts`) | Same. | Same. |
