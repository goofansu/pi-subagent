...
**Ticket agents** are Herdr pi agents, each a main agent in its own worktree and pane, prompted with `/implement-and-review <ticket>`, each ticket being that agent's specification. You orchestrate them; they implement, review, and commit their own ticket.
