...
**Ticket agents** are Herdr pi agents, each a main agent in its own worktree and pane, prompted with `/skill:implement-and-review <ticket>`. You orchestrate them; they implement, review, and commit their own ticket. Use the herdr skill for the commands.
...
4. Work the frontier until every ticket is merged:
   ...
   - Every prompt starts with `/skill:implement-and-review`, then the ticket and pointers to the spec and the exploration notes: `/skill:implement-and-review <ticket> against <spec>, notes in <notes-dir>`.
