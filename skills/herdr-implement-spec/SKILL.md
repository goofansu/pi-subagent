...
Apply the `artifact-handoff` skill to the ticket-agent fan-out. One external handoff root belongs to this orchestration, and each ticket gets one artifact whose fixed header is the fan-in interface. A ticket agent's final message is only the artifact receipt.
...
4. Work the frontier until every ticket is merged into its delivery branch:
   ...
   - Every prompt starts with `/skill:implement-and-review`, then the ticket and pointers to the specification and exploration notes, followed by the complete writer contract from `artifact-handoff`: `/skill:implement-and-review <ticket> against <specification>, notes in <notes-dir>, <writer-contract>`.
   - Wait on the agents. A settled agent has stopped, which is a signal to inspect rather than proof of success: it may have ended clean, or stopped on a specification gap or review stalemate. Read every artifact's fixed header, then open only non-passing or attention-marked artifacts and the evidence needed for verification.
   - As each ticket finishes green, verify its recorded checks, merge its branch into its delivery branch, then remove its temporary worktree and delete its temporary branch. A ticket is green only when the agent stopped successfully, its artifact says `STATUS: pass`, and required claims verify. Merge into any one delivery branch one ticket at a time; the other ticket agents keep running.
...
7. Report the external handoff root, every ticket and which delivery branch contains it, plus anything left unstarted and why. Include each artifact's status and anything unverified. Report every delivery branch and the result of its handoff: the local base and merge commit, or the worktree path and PR when applicable. Ticket branches and worktrees are temporary: none remain at the end. Successfully local-merged delivery branches and worktrees are temporary too. PR delivery branches and worktrees are final: preserve every one.
