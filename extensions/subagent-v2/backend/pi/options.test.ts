...
function emptyAgentDir(t: { after(fn: () => void): void }): string {
  const root = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "pi-subagent-v2-agentdir-")),
  );
...
test("a package identity is read from the nearest manifest above a file", (t) => {
  ...
  assert.equal(
    packageNameForPath(own.resolve("extensions/subagent-v2/index.ts")),
    "pi-subagent",
  );
  ...
  assert.equal(
    packageNameForPath(own.resolve("extensions/subagent-v2/gone.ts")),
    "pi-subagent",
  );
});
