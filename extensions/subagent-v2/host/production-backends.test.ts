...
test("a Session built from the production set validates a Profile naming either backend", async () => {
  ...
  // Both Profiles loaded, so neither backend name is unknown to the set.
...
test("a Profile naming a backend the set does not hold is a diagnostic, not a crash", () => {
  ...
  // Codex arrives at M6. Until then a Profile naming it is a mistake in a
  // file, and the answer is to say which file and which name.
  assert.deepEqual(
  ...
  // Both the set's own backends validate rather than being unknown.
  assert.deepEqual(
    catalog.validateProfile(profileNaming("pi"), "/agents/pi-worker.md"),
    [],
  );
  assert.deepEqual(
