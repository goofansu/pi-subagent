...
import { presentCancelOutcomes, presentWait } from "../presentation/index.ts";
...
test("registered cancellation renderers cover admission outcomes in both expansion states", async (t) => {
  ...
  assert.equal(resultText(emptyResult), "No run ids were given.");
  ...
  assert.match(oneRow.collapsed.result[0] ?? "", /Cancellation requested: 1/);
...
test("agent_wait renders an unknown id distinctly in both expansion states", async (t) => {
  const rig = hostRig(t);
  await rig.host.sessionStart();
  t.after(() => rig.installation.handle.release());
  const args = { ids: ["run-never"] };
  const result = await rig.call("agent_wait", args);
...
test("agent_wait is repeatable, and the Result stays readable afterwards", async (t) => {
  ...
  const started = await startedRun(rig);
  const args = { ids: [started.runId] };
  const first = await rig.call("agent_wait", args);
  const second = await rig.call("agent_wait", args);

  // Reading is not taking: the wait observed the stored Result, and so does
  // every later reader. Repeated reads retain both render forms too.
  assert.deepEqual(first, second);
  for (const expanded of [false, true]) {
    assert.deepEqual(
      renderRegisteredRow(rig.host, "agent_wait", args, first, expanded),
      renderRegisteredRow(rig.host, "agent_wait", args, second, expanded),
    );
  }
  assert.match(
    await rig.text("agent_result", { id: started.runId }),
...
test("aborting the turn ends only the wait: the Run settles and its result stands", async (t) => {
  ...
  const controller = new AbortController();
  const args = { ids: [started.runId] };
  const waiting = rig.call("agent_wait", args, { signal: controller.signal });
  controller.abort();

  const interrupted = await waiting;
  assert.match(resultText(interrupted), new RegExp(started.runId));
  assert.match(resultText(interrupted), /The wait gave up, not the Runs/);
...
test("agent_wait crosses registration with discriminated collection details and complete expansion", async (t) => {
  const rig = hostRig(t);
  await rig.host.sessionStart();
  t.after(() => rig.installation.handle.release());
...
test("agent_wait renders partial mixed-Agent delivery without presenting timeout as completion", async (t) => {
  const rig = hostRig(t, {
    oneShotSteps: [[{ step: "await-gate", gate: "hold" }]],
  });
  await rig.host.sessionStart();
  ...
  const delivered = await startedRun(rig);
  await rig.settled(delivered.runId);
  const running = await startedRun(rig, RIG_ONE_SHOT_PROFILE);
  const args = {
    ids: [delivered.runId, running.runId, "run-never"],
    timeoutSeconds: 0.01,
  };
  const result = await rig.call("agent_wait", args);
...
test("agent_wait_all honours its timeout and reports unfinished Runs", async (t) => {
  ...
  const args = { timeoutSeconds: 0.01 };
...
test("agent_result returns the full stored output with its Run identity", async (t) => {
  ...
  const args = { id: started.runId };
  const result = await rig.call("agent_result", args);
  const text = result.content.map((part) => part.text ?? "").join("");
...
test("agent_result renders a live Run distinctly from unknown in both expansion states", async (t) => {
  const rig = hostRig(t, {
    resumableSteps: [[{ step: "await-gate", gate: "hold" }]],
  });
  await rig.host.sessionStart();
  ...
  const cases = [
    {
      ...
      text:
        `Run ${started.runId} has not finished yet, so it has no result. Its ` +
    ...
    },
    {
      args: { id: "run-never" },
      text:
        "No run with id run-never. Check the id against what agent_start or " +
  ...
  for (const entry of cases) {
    ...
    assert.equal(resultText(result), entry.text);
    ...
    assert.ok(collapsed.result.join("\n").includes(entry.rowPhrase));
    const expanded = renderRegisteredRow(
      rig.host,
      "agent_result",
      entry.args,
...
test("a Result the store evicted tells the host nothing either", async (t) => {
  ...
  const waitArgs = { ids: [first.runId] };
  const waited = await rig.call("agent_wait", waitArgs);
  assert.deepEqual(waited.details, {
    kind: "collection",
    scope: "named",
    runs: [],
    stillRunning: 0,
  ...
  const collapsedWait = renderRegisteredRow(
    rig.host,
    "agent_wait",
    waitArgs,
    waited,
  ...
  assert.match(collapsedWait.result.join("\n"), /1 Result unavailable/);
  ...
  assert.match(collapsedResult.result.join("\n"), /Result unavailable/);
...
test("a Run on the collapsed line alone is not recorded as consumed", async () => {
  ...
  assert.equal(resultText(result), "the answer");
  assert.deepEqual(result.details, {
    kind: "result",
    outcome: "available",
    rowPhrase,
  ...
  for (const entry of cases) {
    ...
    for (const expanded of [false, true]) {
      ...
      if (expanded) {
        ...
        assert.ok((row.result[0] ?? "").includes(entry.rowPhrase));
      }
      assert.ok(
