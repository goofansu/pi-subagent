  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (params.chain && params.chain.length > 0) {
        ...
        const makeChainDetails = makeDetails("chain", totalSteps);
            ...
            ? (partial) => {
                ...
                if (currentResult) {
                  ...
                  onUpdate({
                    ...
                    details: makeChainDetails(allResults),
                  });
                }
              }
            : undefined;
    ...
    renderResult(result, { expanded, isPartial }, theme, _context) {
      ...
      if (details.mode === "single" && details.results.length === 1) {
        ...
        if (isPartial) {
          text += ` ${theme.fg("warning", `(${r.usage.turns} turn${r.usage.turns !== 1 ? "s" : ""}, ${toolCallCount} call${toolCallCount !== 1 ? "s" : ""})`)}`;
      ...
      if (details.mode === "chain") {
        ...
        if (expanded) {
          ...
          for (const r of details.results) {
            const isLastAndRunning =
              isPartial && r === details.results[details.results.length - 1];
            const rIcon = isLastAndRunning
              ? theme.fg("warning", "⏳")
              : r.exitCode === 0
                ? theme.fg("success", "✓")
                : theme.fg("error", "✗");
