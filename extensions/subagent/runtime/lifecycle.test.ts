...
test("a cancelled execution overrun seals intake before execution-scope disposal", async () => {
  const policy: RuntimePolicy = {
    ...DEFAULT_RUNTIME_POLICY,
    cleanupBudgetMillis: 2_000,
  };
  ...
  assert.equal(outcome.value.read.outcome, "result");
  if (outcome.value.read.outcome === "result") {
    assert.equal(outcome.value.read.result.finalOutput, "partial output");
    assert.deepEqual(
      outcome.value.read.result.diagnostics.map((item) => item.category),
      ["cleanup-escalation"],
    );
  }
  // The execution wait spent the one budget. Its scope began closing only at
  // settlement step 4, after sealing, so the finalizer's emission was late.
