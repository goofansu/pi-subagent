/** Pi retained SDK Conversation tests. */
...
test("retained Pi SDK sessions translate tool calls and results into neutral Facts", async () => {
  const listeners = new Set<(event: AgentSessionEvent) => void>();
  const messages: unknown[] = [];
  const toolCall = {
    role: "assistant",
  ...
  const result = await startSubagent({
    ...
    description: "translate retained SDK tools",
...
test("retained Pi SDK terminal message errors fail with a confined diagnostic", async () => {
  const listeners = new Set<(event: AgentSessionEvent) => void>();
  const messages: unknown[] = [];
  const providerSecret = "pi-provider-terminal-secret";
  const terminalError = {
    role: "assistant",
    content: [],
    stopReason: "error",
    errorMessage: `request rejected: ${providerSecret}`,
  };
  const emit = (event: unknown): void => {
    for (const listener of listeners) listener(event as AgentSessionEvent);
  };
  const session: PiSession = {
    ...
    isIdle: true,
    async prompt() {
      messages.push(terminalError);
      emit({ type: "message_end", message: terminalError });
      emit({ type: "agent_end", messages: [...messages], willRetry: false });
    },
    async steer() {},
    subscribe(listener) {
  ...
  const result = await startSubagent({
    config: agent({ harness: "pi" }),
    description: "confine retained terminal error",
    prompt: "do it",
    harnesses: createHarnessRegistry([
      createPiHarness({
        sessionFactory: async () => ({ session }),
  ...
  assert.doesNotMatch(result.errorMessage, new RegExp(providerSecret));
  assert.doesNotMatch(
    result.errorMessage,
    /Pi (?:initialization|prompt) failed/,
  );
});
